...
// @version      0.2.9
// @description  Torn cash-flow, spending, earnings, net-worth and trade analytics with a Bento dashboard, restored proven floating launcher, TCT daily flow and fast sync modes. Data stays on-device.
...
(() => {
  ...
  function injectCss() {
    ...
    s.textContent = `
      ...
      #tta-root,#tta-root *,#tcfa-launcher,#tcfa-launcher *{box-sizing:border-box}
      #tta-root button,#tcfa-launcher{font-family:inherit;-webkit-appearance:none;appearance:none;margin:0;line-height:1.15;touch-action:manipulation;-webkit-tap-highlight-color:transparent}
  ...
  function mount() {
    ...
    document.getElementById('tcfa-launcher')?.remove();document.getElementById('tta-root')?.remove();document.getElementById('tta-fab-host')?.remove();
  ...
  const boot=()=>{if(document.body){mount();installFabWatchdog();resumePendingSync();}else setTimeout(boot,250)}; boot();
})();
