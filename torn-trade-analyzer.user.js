...
(() => {
  ...
  function resetHistoryForFullResync() {
    stripSyncRunMarkers();
    state.transactions=[];state.cashFlows=[];state.playerTransfers=[];state.unrecognizedFinancial=[];
    save('transactions',[]);save('cashFlows',[]);save('playerTransfers',[]);save('unrecognizedFinancial',[]);
