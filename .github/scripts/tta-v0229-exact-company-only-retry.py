...
if start < 0 or end < 0: raise SystemExit('Historical company helper block not found')
s=s[:start]+s[end:]
...
"""
if old_http in s: s=s.replace(old_http,new_http,1)
...
s=s.replace(migration_anchor,migration_anchor+"  try{localStorage.removeItem(NS+('company'+'History'));}catch(_){}\n",1)

if any(ord(ch)>=128 for ch in s): raise SystemExit('Non-ASCII characters introduced')
if 'companyHistory' in s: raise SystemExit('companyHistory references remain')
if '/company/snapshot' in s or 'employees,snapshot' in s: raise SystemExit('company snapshot references remain')
